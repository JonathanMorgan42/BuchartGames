    ...
    render() {
        ...
        this.container.innerHTML = `
            <div class="team-selector">
                <label class="team-selector-label">Select Your Team</label>
...
class GamePlacementInput {
            ...
            <div class="game-simulation-card" data-game-id="${game.id}">
                <div class="game-simulation-header">
                    ...
                    <div class="game-actions">
                        <button class="btn-reset-game" data-game-id="${game.id}">Reset</button>
                        <button class="btn-randomize-game" data-game-id="${game.id}">Random</button>
                    </div>
                </div>
    ...
    attachEventListeners() {
        ...
        // Reset game buttons
        this.container.querySelectorAll('.btn-reset-game').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const gameId = parseInt(e.target.dataset.gameId);
                // Reset to standings order
                this.state.teams.forEach((team, idx) => {
                    this.state.setPlacement(gameId, team.id, idx + 1);
                });
            });
        });

        // Randomize game buttons
        this.container.querySelectorAll('.btn-randomize-game').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const gameId = parseInt(e.target.dataset.gameId);
 ...
 * Plain-language explanation of current scenario
 */
class OutcomeSummary {
    ...
    render() {
        ...
        if (selectedRank === 1) {
            summaryText = `🎉 <strong>${selectedTeam.name} wins!</strong> Final score: ${selectedTeam.finalPoints} pts`;
            summaryClass = 'outcome-win';
        } else {
            const pointsBehind = leader.finalPoints - selectedTeam.finalPoints;
            summaryText = `${selectedTeam.name} finishes ${this.getOrdinal(selectedRank)} with ${selectedTeam.finalPoints} pts (${pointsBehind} behind ${leader.name})`;
 ...
 * Finds and displays paths to victory
 */
class WinSimulator {
    constructor(container, state) {
        this.container = container;
    ...
    computePath() {
        ...
        resultsContainer.innerHTML = '<div class="loading">Computing...</div>';
            ...
            if (!scenario) {
                resultsContainer.innerHTML = `
                    <div class="path-results no-path">
                        <p>❌ <strong>No path to 1st place found.</strong></p>
                    </div>
                `;
            } else {
...
/**
 * Share Scenario Component
 * Allows copying scenario description
 */
class ShareScenario {
    constructor(container, state) {
        this.container = container;
    ...
    render() {
        this.container.innerHTML = `
            <button class="btn-share" id="shareScenarioBtn">📋 Share Scenario</button>
        `;

        const btn = this.container.querySelector('#shareScenarioBtn');
        btn.addEventListener('click', () => this.shareScenario());
    }
    ...
    shareScenario() {
        ...
        let scenarioText = `Game Night Scenario - ${selectedTeam.name}\n\n`;
        scenarioText += `Projected Results:\n`;
        results.forEach((team, idx) => {
            scenarioText += `${idx + 1}. ${team.name}: ${team.finalPoints} pts\n`;
        });

        scenarioText += `\nGame Placements:\n`;
        this.state.games.forEach(game => {
            scenarioText += `\n${game.name}:\n`;
            const placements = this.state.placements[game.id];
            const sortedPlacements = Object.entries(placements)
                .sort(([, a], [, b]) => a - b);

            sortedPlacements.forEach(([teamId, place]) => {
                const team = this.state.teams.find(t => t.id === parseInt(teamId));
                scenarioText += `  ${place}. ${team.name}\n`;
            });
        });
...
function initPlayground(teams, games) {
    ...
    const shareScenario = new ShareScenario(
        document.getElementById('shareScenarioContainer'),
        state
    );
