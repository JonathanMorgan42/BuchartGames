...
class TestGameNightForm:
    ...
    # BVA: Maximum name length (200 characters - boundary)
    def test_maximum_name_length(self, app):
        """Test maximum valid name length (exactly 200) - BVA."""
        with app.test_request_context():
            form = GameNightForm(data={
                'name': 'A' * 200,  # Exactly 200 chars
                'date': date.today()
            })
            assert form.validate() is True

    # BVA: Above maximum name length (201 characters)
    def test_above_maximum_name_length(self, app):
        """Test above maximum name length (201 chars) - BVA."""
        with app.test_request_context():
            form = GameNightForm(data={
                'name': 'A' * 201,  # 201 chars - too long
                'date': date.today()
            })
            assert form.validate() is False
            assert 'name' in form.errors

    # BVA: Near maximum name length (199 characters)
    def test_near_maximum_name_length(self, app):
        """Test near maximum name length (199 chars) - BVA."""
        with app.test_request_context():
            form = GameNightForm(data={
                'name': 'A' * 199,
                'date': date.today()
